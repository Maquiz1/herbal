...
class CRF6CreateUpdateView(CreateUpdateView,LoginRequiredMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not kwargs.get('pk'):
            ...
            if visit_id:
                self.preselected_visit = get_object_or_404(
                    Visit,
                    pk=visit_id,
                    visit_type='DAY120'  # Enforce only Day 120
                )
    ...
    def get_context_data(self, **kwargs):
        ...
        if visit:
            ...
            context['title'] = f"{'Edit' if self.object else 'Complete'} Study - {visit.enrollment.patient.patient.pid}"
        return context

    def form_valid(self, form):
        messages.success(self.request, "Study completion recorded successfully.")
